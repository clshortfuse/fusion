 ...
 * complete}
 */
public class CustomViewPagerAdapterV8 extends PagerAdapter {
	...
	static final class PageInfo {
		...
		PageInfo(Class<?> _class, Bundle _args) {
			clss = _class;
			args = _args;
		}
	}
	...
	public CustomViewPagerAdapterV8(Context mContext, FragmentManager fm) {
		mFragmentManager = fm;
	...
	public void insertPage(Class<?> clss, Bundle args, int position) {
		PageInfo info = new PageInfo(clss, args);
		mPages.add(position, info);
	...
	public void removePageAt(int position) {
		...
		// this.destroyItem(null, position, this.mFragments.get(position));
	...
	public CharSequence getPageTitle(int position) {
		...
		if (info.clss == ContactsFragment.class)
			return "Contacts";
		else {
			Fragment f = mFragments.get(position);
			ContactItem c = null;
			if (f != null && f instanceof ConversationFragment)
				c = ((ConversationFragment) f).getContactItem();
			if (c == null)
				c = info.args.getParcelable("contactItem");
