...
public final class Telephony {
	...
	public static final class Sms implements BaseColumns, TextBasedSmsColumns {
		...
		public static final class Outbox implements BaseColumns,
			 ...
			 *            the timestamp for the message
			 * @param deliveryReport
			 *            whether a delivery report was requested for the
			 *            message
			 * @return the URI for the new message
			 */
			public static Uri addMessage(ContentResolver resolver,
		...
		public static final class Intents {
			...
			/**
			 * Set by BroadcastReceiver. Indicates the message, while possibly
			 * valid, is of a format or encoding that is not supported.
			 */
			 ...
			 * 
			 * <p>
			 * If a BroadcastReceiver encounters an error while processing this
			 * intent it should set the result code appropriately.
			 * </p>
			 */
			 ...
			 * device. The intent will have the following extra values:</p>
			 * 
			 * <ul>
			 ...
			 * - Any parameters associated with the content type (decoded from
			 ...
			 * 
			 * <p>
			 * If any unassigned well-known parameters are encountered, the key
			 * of the map will be 'unassigned/0x...', where '...' is the hex
			 * value of the unassigned parameter. If a parameter has No-Value
			 * the value in the map will be null.
			 * </p>
			 */
			 ...
			 * 
			 * <ul>
			 * <li><em>message</em> - An SmsCbMessage object containing the
			 * broadcast message data. This is not an emergency alert, so ETWS
			 * and CMAS data will be null.</li>
			 * </ul>
			 * 
			 * <p>
			 * The extra values can be extracted using
			...
			/**
			 * Broadcast Action: A new Emergency Broadcast message has been
			 * received by the device. The intent will have the following extra
			 * values:</p>
			 * 
			 * <ul>
			 * <li><em>message</em> - An SmsCbMessage object containing the
			 * broadcast message data, including ETWS or CMAS warning
			 * notification info if present.</li>
			 * </ul>
			 * 
			 * <p>
			 * The extra values can be extracted using
			...
			/**
			 * Broadcast Action: A new CDMA SMS has been received containing
			 * Service Category Program Data (updates the list of enabled
			 * broadcast channels). The intent will have the following extra
			 * values:</p>
			 * 
			 * <ul>
			 * <li><em>operations</em> - An array of CdmaSmsCbProgramData
			 * objects containing the service category operations
			 * (add/delete/clear) to perform.</li>
			 * </ul>
			 * 
			 * <p>
			 * The extra values can be extracted using
			 ...
			 * telephony framework. This intent is sent in lieu of any of the
			 * RECEIVED_ACTION intents. The intent will have the following extra
			 * value:</p>
			 * 
			 * <ul>
			 * <li><em>result</em> - An int result code, eg,
			 * <code>{@link #RESULT_SMS_OUT_OF_MEMORY}</code>, indicating the
			 * error returned to the network.</li>
			 * </ul>
			 */
			...
			public static SmsMessage[] getMessagesFromIntent(Intent intent) {
				...
				// TODO: Was createFromPdu(pdu[i], format);
				return msgs;
			}
		}
	}
	...
	public interface BaseMmsColumns extends BaseColumns {
		...
		/**
		 * Indicates whether this message has only a text part (can also have a
		 * subject) and no picture, slideshow, or sound, etc., parts. The value
		 * is a boolean, 1 or 0.
		 */
	...
	public interface CanonicalAddressesColumns extends BaseColumns {
		/**
		 * An address used in MMS or SMS. Email addresses are converted to lower
		 * case and are compared by string equality. Other addresses are
		 * compared using PHONE_NUMBERS_EQUAL.
		 * <P>
		 * Type: TEXT
		 * </P>
		 */
	...
	public static final class Threads implements ThreadsColumns {
		...
		public static long getOrCreateThreadId(Context context,
			...
			Cursor cursor = SqliteWrapper.query(context,
					context.getContentResolver(), uri, ID_PROJECTION, null,
					null, null);
			if (cursor != null) {
				try {
	...
	public static final class Mms implements BaseMmsColumns {
		...
		public static final String getMessageBoxName(int msgBox) {
			...
			case MESSAGE_BOX_OUTBOX:
				return "outbox";
			default:
				throw new IllegalArgumentException("Invalid message box: "
						+ msgBox);
			}
		}
		...
		public static final class Intents {
			...
			/**
			 * The extra field to store the contents of the Intent, which should
			 * be an array of Uri.
			 */
			public static final String EXTRA_CONTENTS = "contents";
			/**
			 * The extra field to store the type of the contents, which should
			...
			/**
			 * Indicates that the contents of specified URIs were changed. The
			 * application which is showing or caching these contents should be
			 * updated.
			 */
	...
	public static final class Carriers implements BaseColumns {
		 ...
		 * 
		 * One of the PDP_type values in TS 27.007 section 10.1.1. For example,
		...
		/**
		 * Current status of APN true : enabled APN, false : disabled APN.
		 */
		public static final String CARRIER_ENABLED = "carrier_enabled";

		/**
		 * Radio Access Technology info To check what values can hold, refer to
		 * ServiceState.java. This should be spread to other technologies, but
		 * currently only used for LTE(14) and EHRPD(13).
		 */
	...
	public static final class CellBroadcasts implements BaseColumns {
		...
		/**
		 * PLMN of broadcast sender. (SERIAL_NUMBER + PLMN + LAC + CID) uniquely
		 * identifies a broadcast for duplicate detection purposes.
		 * <P>
		 * Type: TEXT
		 * </P>
		 */
		 ...
		 * Unused for CDMA. Only included if Geographical Scope of message is
		 * not PLMN wide (01).
		 * <P>
		 * Type: INTEGER
		 * </P>
		...
		/**
		 * Cell ID of message sender (GSM/UMTS). Unused for CDMA. Only included
		 * when the Geographical Scope of message is cell wide (00 or 11).
		 * <P>
		 * Type: INTEGER
		 * </P>
		 */
		...
		/**
		 * Service category (GSM/UMTS message identifier, CDMA service
		 * category).
		 * <P>
		 * Type: INTEGER
		 * </P>
		 ...
		 */
		public static final String[] QUERY_COLUMNS = { _ID, GEOGRAPHICAL_SCOPE,
				PLMN, LAC, CID, SERIAL_NUMBER, SERVICE_CATEGORY, LANGUAGE_CODE,
				MESSAGE_BODY, DELIVERY_TIME, MESSAGE_READ, MESSAGE_FORMAT,
				MESSAGE_PRIORITY, ETWS_WARNING_TYPE, CMAS_MESSAGE_CLASS,
				CMAS_CATEGORY, CMAS_RESPONSE_TYPE, CMAS_SEVERITY, CMAS_URGENCY,
				CMAS_CERTAINTY };
	}
}