	...
	protected PduHeaders parseHeaders(ByteArrayInputStream pduDataStream) {
		...
		while (keepParsing && (pduDataStream.available() > 0)) {
			...
			case PduHeaders.RESPONSE_STATUS: {
				...
				if (LOCAL_LOGV) {
					Log.v(LOG_TAG, "parseHeaders: byte: " + headerField
							+ " value: " + value);
				}
			...
			case PduHeaders.MESSAGE_SIZE: {
				try {
					...
					if (LOCAL_LOGV) {
						Log.v(LOG_TAG, "parseHeaders: longint: " + headerField
								+ " value: " + value);
					}
			...
			case PduHeaders.LIMIT: {
				try {
					...
					if (LOCAL_LOGV) {
						Log.v(LOG_TAG, "parseHeaders: int: " + headerField
								+ " value: " + value);
					}
					headers.setLongInteger(value, headerField);
				} catch (RuntimeException e) {
			...
			case PduHeaders.CONTENT_LOCATION: {
				...
				if (null != value) {
					try {
						if (LOCAL_LOGV) {
							Log.v(LOG_TAG, "parseHeaders: string: "
									+ headerField + " value: "
									+ new String(value));
						}
			...
			case PduHeaders.RESPONSE_TEXT: {
				...
				if (null != value) {
					try {
						...
						log(headerField
								+ "is not Encoded-String-Value header field!");
						return null;
					}
				}
				break;
			...
			case PduHeaders.TO: {
				...
				if (null != value) {
					...
					if (null != address) {
						...
						if (LOCAL_LOGV) {
							Log.v(LOG_TAG,
									"parseHeaders: (to/cc/bcc) address: "
			...
			case PduHeaders.REPLY_CHARGING_DEADLINE: {
				...
				try {
					if (LOCAL_LOGV) {
						Log.v(LOG_TAG, "parseHeaders: time value: "
								+ headerField + " value: " + timeValue);
					}
			...
			case PduHeaders.FROM: {
				...
				try {
					if (LOCAL_LOGV) {
						Log.v(LOG_TAG, "parseHeaders: from address: "
								+ headerField + " value: " + from.getString());
					}
			...
			case PduHeaders.MESSAGE_CLASS: {
				...
				if (LOCAL_LOGV) {
					Log.v(LOG_TAG, "parseHeaders: MESSAGE_CLASS: "
							+ headerField + " value: " + messageClass);
				}
				...
				if (messageClass >= PduHeaders.MESSAGE_CLASS_PERSONAL) {
					...
					/* Token-text */
					pduDataStream.reset();
					byte[] messageClassString = parseWapString(pduDataStream,
							TYPE_TEXT_STRING);
					if (null != messageClassString) {
						try {
							...
							log(headerField
									+ "is not Text-String header field!");
							return null;
						}
					}
				}
			...
			case PduHeaders.MMS_VERSION: {
				...
				try {
					if (LOCAL_LOGV) {
						Log.v(LOG_TAG, "parseHeaders: MMS_VERSION: "
								+ headerField + " value: " + version);
					}
			...
			case PduHeaders.PREVIOUSLY_SENT_BY: {
				...
				if (null != previouslySentBy) {
					try {
						if (LOCAL_LOGV) {
							Log.v(LOG_TAG, "parseHeaders: PREVIOUSLY_SENT_BY: "
									+ headerField + " value: "
									+ previouslySentBy.getString());
						}
			...
			case PduHeaders.PREVIOUSLY_SENT_DATE: {
				...
				try {
					...
					if (LOCAL_LOGV) {
						Log.v(LOG_TAG, "parseHeaders: PREVIOUSLY_SENT_DATE: "
								+ headerField + " value: " + perviouslySentDate);
					}
			...
			case PduHeaders.ELEMENT_DESCRIPTOR: {
				if (LOCAL_LOGV) {
					Log.v(LOG_TAG, "parseHeaders: ELEMENT_DESCRIPTOR: "
							+ headerField);
				}
			...
			case PduHeaders.CONTENT_TYPE: {
				...
				if (null != contentType) {
					try {
						if (LOCAL_LOGV) {
							Log.v(LOG_TAG, "parseHeaders: CONTENT_TYPE: "
									+ headerField + contentType.toString());
						}
						headers.setTextString(contentType,
			...
			default: {
				if (LOCAL_LOGV) {
					Log.v(LOG_TAG, "parseHeaders: Unknown header: "
							+ headerField);
				}
				log("Unknown header");
			}
	...
	protected static PduBody parseParts(ByteArrayInputStream pduDataStream) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (null != contentType) {
				...
				part.setContentType((PduContentTypes.contentTypes[0])
						.getBytes()); // "*/*"
			}
			...
			if (partHeaderLen > 0) {
				if (false == parsePartHeaders(pduDataStream, part,
						partHeaderLen)) {
			 ...
			 */
			if ((null == part.getContentLocation()) && (null == part.getName())
			...
			if (dataLength > 0) {
				...
				if (partContentType
						.equalsIgnoreCase(ContentType.MULTIPART_ALTERNATIVE)) {
					// parse "multipart/vnd.wap.multipart.alternative".
					PduBody childBody = parseParts(new ByteArrayInputStream(
							partData));
					...
					if (null != partDataEncoding) {
						...
						if (encoding.equalsIgnoreCase(PduPart.P_BASE64)) {
							...
							partData = Base64.decodeBase64(partData);
								...
								.equalsIgnoreCase(PduPart.P_QUOTED_PRINTABLE)) {
							// Decode "quoted-printable" into "binary".
							partData = QuotedPrintable
									.decodeQuotedPrintable(partData);
						} else {
...
/**
	 ...
	 */
	protected static int skipWapValue(ByteArrayInputStream pduDataStream,
			int length) {
		assert (null != pduDataStream);
		...
		while (0 < lastLen) {
			...
			case PduPart.P_CT_MR_TYPE:
				...
				if (first > TEXT_MAX) {
					...
					if (index < PduContentTypes.contentTypes.length) {
						byte[] type = (PduContentTypes.contentTypes[index])
								.getBytes();
					...
					byte[] type = parseWapString(pduDataStream,
							TYPE_TEXT_STRING);
			...
			case PduPart.P_CHARSET:
				...
				if (((firstValue > TEXT_MIN) && (firstValue < TEXT_MAX))
					...
					byte[] charsetStr = parseWapString(pduDataStream,
							TYPE_TEXT_STRING);
					try {
						int charsetInt = CharacterSets
								.getMibEnumValue(new String(charsetStr));
		...
		if (cur < TEXT_MIN) {
			...
			if ((first >= TEXT_MIN) && (first <= TEXT_MAX)) {
				...
				if (index < PduContentTypes.contentTypes.length) { // well-known
																	// type
					contentType = (PduContentTypes.contentTypes[index])
							.getBytes();
				} else {
					pduDataStream.reset();
					contentType = parseWapString(pduDataStream,
							TYPE_TEXT_STRING);
				}
			} else {
				Log.e(LOG_TAG, "Corrupt content-type");
			...
			contentType = (PduContentTypes.contentTypes[parseShortInteger(pduDataStream)])
					.getBytes();
		}
		...
		while (0 < lastLen) {
			...
			if (header > TEXT_MAX) {
				...
				case PduPart.P_CONTENT_LOCATION:
					 ...
					 */
					byte[] contentLocation = parseWapString(pduDataStream,
							TYPE_TEXT_STRING);
				...
				case PduPart.P_CONTENT_ID:
					 ...
					 */
					byte[] contentId = parseWapString(pduDataStream,
							TYPE_QUOTED_STRING);
					if (null != contentId) {
				...
				case PduPart.P_CONTENT_DISPOSITION:
					 ...
					 * Inline | Token-text Form-data = <Octet 128> Attachment =
					 * <Octet 129> Inline = <Octet 130>
					 */
					...
					//TODO: User selectable Content Disposition
					...
					if (contentDisposition) {
						...
						if (value == PduPart.P_DISPOSITION_FROM_DATA) {
							...
							pduDataStream.reset();
							/* Token-text */
							part.setContentDisposition(parseWapString(
									pduDataStream, TYPE_TEXT_STRING));
						}
				...
				// Not assigned header.
				byte[] tempHeader = parseWapString(pduDataStream,
						TYPE_TEXT_STRING);
				byte[] tempValue = parseWapString(pduDataStream,
						TYPE_TEXT_STRING);

				// Check the header whether it is "Content-Transfer-Encoding".
				if (true == PduPart.CONTENT_TRANSFER_ENCODING
	...
	protected static boolean checkMandatoryHeader(PduHeaders headers) {
		...
		case PduHeaders.MESSAGE_TYPE_SEND_REQ:
			...
			// From field.
			EncodedStringValue srFrom = headers
		...
		case PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND:
			...
			byte[] niContentLocation = headers
		...
		case PduHeaders.MESSAGE_TYPE_DELIVERY_IND:
			...
			// To field.
			EncodedStringValue[] diTo = headers
		...
		case PduHeaders.MESSAGE_TYPE_READ_ORIG_IND:
			...
			// From field.
			EncodedStringValue roFrom = headers
					.getEncodedStringValue(PduHeaders.FROM);
			if (null == roFrom) {
				return false;
			}
			...
			// To field.
			EncodedStringValue[] roTo = headers
					.getEncodedStringValues(PduHeaders.TO);
			if (null == roTo) {
				return false;
			}
		...
		case PduHeaders.MESSAGE_TYPE_READ_REC_IND:
			// From field.
			EncodedStringValue rrFrom = headers
					.getEncodedStringValue(PduHeaders.FROM);
			if (null == rrFrom) {
				return false;
			}
			...
			// To field.
			EncodedStringValue[] rrTo = headers
					.getEncodedStringValues(PduHeaders.TO);
			if (null == rrTo) {
				return false;
			}
