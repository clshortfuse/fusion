...
public class PduHeaders {
	...
	protected void setOctet(int value, int field)
		...
		case RETRIEVE_STATUS:
			// According to oma-ts-mms-enc-v1_3, section 7.3.50, we modify the
			// invalid value.
			...
			break;
		case STORE_STATUS:
			// According to oma-ts-mms-enc-v1_3, section 7.3.58, we modify the
			// invalid value.
		...
		case RESPONSE_STATUS:
			// According to oma-ts-mms-enc-v1_3, section 7.3.48, we modify the
			// invalid value.
		...
		case MMS_VERSION:
			if ((value < MMS_VERSION_1_0) || (value > MMS_VERSION_1_3)) {
				value = CURRENT_MMS_VERSION; // Current version is the default
												// value.
			}
			break;
		case MESSAGE_TYPE:
